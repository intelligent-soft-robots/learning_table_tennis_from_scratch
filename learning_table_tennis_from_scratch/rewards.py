...
def compute_rewards(reward_function, target, nb_balls, observations, episode=None):
    ...
    states = [ [obs_state.get(index) for index in range(nb_balls)] for obs_state in states]
    positions_velocities = numpy.array( [ [(ball.get_position(),ball.get_velocity())
                                            for ball in entry] for entry in states ] )
    ...
    contacts = numpy.array( [[ext_state.contacts[index] for index in range(nb_balls)]
                               for ext_state in ext_states] ).T
