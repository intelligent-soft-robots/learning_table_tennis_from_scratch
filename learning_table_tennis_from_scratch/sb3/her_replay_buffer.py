    ...
    def __init__(
        ...
        if self.apply_HSM:
            input_shape = {
                ...
                "hindsight_previous_obs": (self.env.num_envs,) + (self.HSM_shape,) + self.obs_shape,
                "hindsight_rewards": (self.env.num_envs,) + (self.HSM_shape,),
                "hindsight_dones": (self.env.num_envs,) + (self.HSM_shape,),
                "hindsight_action": (self.action_dim,),
                "action": (self.action_dim,),
                "reward": (1,),
                "next_obs": (self.env.num_envs,) + self.obs_shape,
                "hindsight_obs": (self.env.num_envs,) + (self.HSM_shape,) + self.obs_shape,
                "done": (1,),
            }
            ...
            for hsm_trajectory in self.hsm_traj_buffer:
                for idx_trans in range(len(hsm_trajectory)):
                    ...
                    if self.hindsight_state_selection_strategy == HindsightStateSelectionStrategy.RANDOM:
                        ...
                        ob, next_ob, action, reward, _, _ = transition
                        if self.hindsight_state_selection_strategy_horizon == HindsightStateSelectionStrategyHorizon.STEP:
                            ob_1 = ob
                            ob_2 = next_ob
                            reward_sum = reward
            ...
            if self.apply_HSM:
                for info in infos:
                    if "trajectory" in info:
                        self.add_hindsight_transitions(
