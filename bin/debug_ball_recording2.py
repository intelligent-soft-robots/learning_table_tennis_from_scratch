...
if __name__ == "__main__":
    hysr_config = HysrOneBallConfig.from_json(
        Path(__file__).parents[1] / "config" / "hysr_config.json"
    )
    simulated_robot_handle = configure_mujoco.configure_simulation(hysr_config)
    parallel_burst = pam_mujoco.mirroring.ParallelBurst(
        [simulated_robot_handle.interfaces["robot"]]
    )
    nb_sim_bursts = int(hysr_config.algo_time_step / hysr_config.mujoco_time_step)
    ...
    # fake_timesteps = np.arange(
    #     0, 150 * fake_recording_timestep, fake_recording_timestep
    # )
    # fake_positions = np.stack(3 * [np.arange(1, 151, dtype=np.float32)], axis=1)
    ...
    frequency_manager = o80.FrequencyManager(1.0 / hysr_config.algo_time_step)
    print("\n---------------------\n")
    for _ in range(150):
