...
# Plotting
fig, axes = plt.subplots(3, 1, figsize=(15, 10))  # 2,3
...
match_color_pairs = [('/tmp/ep_20240625-172917', 'blue'), ('/tmp/ep_20240625-162825', 'green')]
...
for filename_match, color in match_color_pairs:
    ...
    for filename in files:
        ...
        axes[0].scatter(timestamps, vel_x, label=file_name, s=20, color=color, alpha=0.5)
        axes[0].set_title("Velocity X")
        axes[1].scatter(timestamps, vel_y, s=20, color=color, alpha=0.5)
        axes[1].set_title("Velocity Y")
        axes[2].scatter(timestamps, vel_z, s=20, color=color, alpha=0.5)
        axes[2].set_title("Velocity Z")

        # Cut off the last timestep since there seems to be a some discontinuity
        vel_fd = np.array(positions)[1:-1] / 0.01 - np.array(positions)[:-2] / 0.01
        timestamps_fd = timestamps[1:-1]
        axes[0].scatter(timestamps_fd, vel_fd[:, 0], label=file_name, s=20, color=color, alpha=0.5, marker="x")
        axes[1].scatter(timestamps_fd, vel_fd[:, 1], s=20, color=color, alpha=0.5, marker="x")
        axes[2].scatter(timestamps_fd, vel_fd[:, 2], s=20, color=color, alpha=0.5, marker="x")
...
# axes[0, 0].scatter(timestamps, trajectory[:, 0], label="recorded ball (direct replay)", s=20, color="red", alpha=0.5)
# axes[0, 0].set_title("Position X")
# axes[0, 1].scatter(timestamps, trajectory[:, 1], s=20, color="red", alpha=0.5)
# axes[0, 1].set_title("Position Y")
# axes[0, 2].scatter(timestamps, trajectory[:, 2], s=20, color="red", alpha=0.5)
...
for filename in files:
    ...
    timestamps_fd = timestamps[1:-1]
    file_name = filename.split('/')[-1]
    axes[0].scatter(timestamps, vel_x, label="real ball (unfiltered)", s=20, color="orange", alpha=0.5)
    axes[1].scatter(timestamps, vel_y, s=20, color="orange", alpha=0.5)
    axes[2].scatter(timestamps, vel_z, s=20, color="orange", alpha=0.5)
    axes[0].scatter(timestamps_fd, vel_fd[:, 0], label="real ball (unfiltered)", s=20, color="orange", alpha=0.5, marker="x")
    axes[1].scatter(timestamps_fd, vel_fd[:, 1], s=20, color="orange", alpha=0.5, marker="x")
    axes[2].scatter(timestamps_fd, vel_fd[:, 2], s=20, color="orange", alpha=0.5, marker="x")
#
axes[-1].legend(["real ball (obs)", "real ball (fd)", "recorded ball (obs)", "recorded ball (fd)", "real ball (unfiltered, obs)", "real ball (unfiltered, fd)"])
plt.show()