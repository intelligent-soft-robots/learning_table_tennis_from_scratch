...
if __name__ == '__main__':
    ...
    #hysr_config.algo_time_step = 0.002
    ...
    #burster = o80.BursterClient("simulation")


    #ball_frontend = o80_pam.BallFrontEnd("ball")
    ball_communication = o80_pam.o80Ball("ball")
    ...
    fake_positions = np.repeat(np.repeat(np.arange(10), 15)[:, None], 3, axis=1) / 30
    ...
    plt.plot(timestamps, trajectory[:, 2], label="o80", marker="x")
    plt.plot(other_timestamps, trajectory[:, 2], label="o80_mujoco_timestamp", marker="x")
    plt.plot(trajectory_orig[0], trajectory_orig[1][:, 2], label="file", marker="x")
