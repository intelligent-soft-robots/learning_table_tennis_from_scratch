...
class StateGoalNetwork(nn.Module):
    def __init__(self, env, dim_out=1, state_embedding=None, goal_embedding=None, layers=[512, 512], max_horizon=None, freeze_embeddings=False, add_extra_conditioning=False, dropout=0):
        ...
        if isinstance(env.observation_space, gym.spaces.Dict):
            ...
            goal_shape = env.observation_space['achieved_goal'].shape
        else:
            obs_shape = env.observation_space.shape
            goal_shape = env.goal_space.shape

        # Create zero tensors with correct shapes
        zero_state = torch.zeros((1,) + obs_shape)
        zero_goal = torch.zeros((1,) + goal_shape)
        state_dim_in = self.state_embedding(zero_state).size()[1]
        goal_dim_in = self.goal_embedding(zero_goal).size()[1]
